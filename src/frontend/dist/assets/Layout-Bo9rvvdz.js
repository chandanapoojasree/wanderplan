...
import { i as Subscribable, p as pendingThenable, k as resolveEnabled, s as shallowEqualObjects, l as resolveStaleTime, n as noop$1, m as environmentManager, o as isValidTimeout, t as timeUntilStale, q as timeoutManager, v as focusManager, w as fetchState, x as replaceData, y as notifyManager, z as hashKey, A as getDefaultState, r as reactExports, B as shouldThrowError, f as useQueryClient, C as useRouterState, j as jsxRuntimeExports, R as React, D as clsx, c as cn, E as o, h as ReactDOM, e as useInternetIdentity, L as Link } from "./index-B3kq8tww.js";
...
const STATIC_DESTINATIONS = [
  {
    ...
    name: "Santorini",
    country: "Greece",
    description: "Iconic white-washed villages perched on volcanic cliffs, world-class sunsets, and the sparkling Aegean Sea make Santorini a timeless dream destination.",
  ...
  },
  {
    ...
    name: "Kyoto",
    country: "Japan",
    description: "Ancient temples, bamboo forests, and cherry blossoms define this cultural capital. Kyoto offers an unparalleled journey through Japan's imperial heritage.",
  ...
  },
  {
    ...
    name: "Amalfi Coast",
    country: "Italy",
    description: "Dramatic cliffs dropping into turquoise waters, pastel-colored villages, and exceptional Italian cuisine make the Amalfi Coast unforgettable.",
  ...
  },
  {
    ...
    name: "Patagonia",
    country: "Argentina",
    description: "At the end of the world lies one of Earth's last wild places. Towering granite peaks, glaciers, and pristine wilderness await the adventurous traveler.",
  ...
  },
  {
    ...
    name: "Bali",
    country: "Indonesia",
    description: "Spiritual temples, lush rice terraces, vibrant arts scene, and world-class surf breaks — Bali offers a rich tapestry of culture and natural beauty.",
  ...
  },
  {
    ...
    name: "Marrakech",
    country: "Morocco",
    description: "A sensory feast of spice-filled souks, ornate palaces, and the legendary Djemaa el-Fna square. Marrakech is where ancient tradition meets modern luxury.",
  ...
  },
  {
    ...
    description: "The city that never sleeps — an electrifying metropolis of iconic skyscrapers, world-class museums, Broadway shows, and an unparalleled culinary scene.",
    imageUrl: "https://source.unsplash.com/featured/?newyorkcity,travel,800x600",
    activities: [
      "Central Park Walk",
      "Broadway Show",
  ...
  },
  {
    ...
    description: "Where the Atlantic meets dramatic mountain peaks, Cape Town dazzles with vineyard visits, penguin colonies, and a vibrant, multicultural spirit unlike anywhere else.",
...
function useDestinations(filter2) {
  return useQuery({
    queryKey: ["destinations", filter2],
    queryFn: async () => {
      let results = [...STATIC_DESTINATIONS];
      if (filter2) {
        if (filter2.minBudget !== void 0) {
          results = results.filter(
            (d) => d.estimatedDailyBudget >= filter2.minBudget
          );
        }
        if (filter2.maxBudget !== void 0) {
          results = results.filter(
            (d) => d.estimatedDailyBudget <= filter2.maxBudget
          );
        }
        if (filter2.minRating !== void 0) {
          results = results.filter((d) => d.rating >= filter2.minRating);
        }
        if (filter2.tags.length > 0) {
          results = results.filter(
            (d) => filter2.tags.some((tag) => d.tags.includes(tag))
          );
        }
      }
      return results;
...
function useDestination(id2) {
  return useQuery({
    queryKey: ["destination", id2 == null ? void 0 : id2.toString()],
    queryFn: async () => {
      if (id2 === void 0) return null;
      return STATIC_DESTINATIONS.find((d) => d.id === id2) ?? null;
    },
    enabled: id2 !== void 0,
    staleTime: Number.POSITIVE_INFINITY
  });
...
function loadTrips() {
  try {
    ...
    return parsed.map((t) => ({
      ...t,
      id: BigInt(t.id),
      destinationId: BigInt(t.destinationId),
      budget: BigInt(t.budget),
      itinerary: (t.itinerary ?? []).map((day) => ({
        ...
        activities: (day.activities ?? []).map((act) => ({
          ...act,
          id: BigInt(act.id),
          durationMinutes: BigInt(act.durationMinutes)
        }))
      }))
    }));
  } catch {
...
function useTrips() {
  ...
  return useQuery({
    queryKey: ["trips", identity == null ? void 0 : identity.getPrincipal().toString()],
    queryFn: async () => {
      if (!identity) return [];
...
function useTrip(id2) {
  const { identity } = useInternetIdentity();
  return useQuery({
    queryKey: ["trip", id2 == null ? void 0 : id2.toString()],
    queryFn: async () => {
      if (id2 === void 0) return null;
      return loadTrips().find((t) => t.id === id2) ?? null;
    },
    enabled: id2 !== void 0 && !!identity
  });
}
  ...
  return useMutation({
    mutationFn: async (profile) => {
      ...
      const saved = {
        ...
        createdAt: existing ? JSON.parse(existing).createdAt : BigInt(Date.now())
      };
...
function NavBar() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "fixed top-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-md border-b border-border shadow-subtle", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "hidden md:flex items-center gap-1", children: NAV_LINKS.map((link) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        Link,
        {
          to: link.to,
          className: `px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive(link.to) ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "hidden md:flex items-center gap-3", children: [
        isAuthenticated && /* @__PURE__ */ jsxRuntimeExports.jsx(
          Link,
          {
            ...
            children: (profile == null ? void 0 : profile.displayName) ?? "Dashboard"
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Button,
          {
            ...
            children: isInitializing ? "Loading…" : isLoggingIn ? "Signing in…" : isAuthenticated ? "Logout" : "Login"
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
    ...
    mobileOpen && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "md:hidden bg-card border-t border-border px-4 pb-4 pt-2 flex flex-col gap-1", children: [
      NAV_LINKS.map((link) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        Link,
        {
          to: link.to,
          onClick: () => setMobileOpen(false),
          className: `px-4 py-2.5 rounded-lg text-sm font-medium transition-smooth ${isActive(link.to) ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`,
...
function Layout({ children, noTopPad = false }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen flex flex-col bg-background", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: `flex-1 ${noTopPad ? "" : "pt-16"}`, children }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "bg-card border-t border-border mt-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-sm text-muted-foreground", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-primary font-display font-semibold text-base", children: "✦ WanderPlan" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-border", children: "·" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Your world, curated." })
      ] }),
...
export {
  ...
  useAddActivity as l,
  motion as m,
  useUpdateActivity as n,
  useRemoveActivity as o,
  useUpdateTripStatus as p,
  composeRefs as q,
  createSlot as r,
  Slot as s,
  cva as t,
  useDestinations as u
};
